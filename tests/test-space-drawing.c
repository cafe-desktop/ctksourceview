...
create_window (void)
{
	...
	hgrid = ctk_grid_new ();
	ctk_orientable_set_orientation (GTK_ORIENTABLE (hgrid), GTK_ORIENTATION_HORIZONTAL);
	...
	ctk_text_view_set_monospace (GTK_TEXT_VIEW (view), TRUE);
	...
	ctk_orientable_set_orientation (GTK_ORIENTABLE (panel_grid), GTK_ORIENTATION_VERTICAL);
	ctk_container_add (GTK_CONTAINER (hgrid), panel_grid);

	ctk_grid_set_row_spacing (GTK_GRID (panel_grid), 6);
	g_object_set (panel_grid,
		      "margin", 6,
		      NULL);

	matrix_checkbutton = ctk_check_button_new_with_label ("GtkSourceSpaceDrawer enable-matrix");
	ctk_container_add (GTK_CONTAINER (panel_grid), matrix_checkbutton);
	ctk_toggle_button_set_active (GTK_TOGGLE_BUTTON (matrix_checkbutton), TRUE);
	...
	ctk_container_add (GTK_CONTAINER (panel_grid), tag_set_checkbutton);
	ctk_toggle_button_set_active (GTK_TOGGLE_BUTTON (tag_set_checkbutton), TRUE);
	...
	ctk_container_add (GTK_CONTAINER (panel_grid), tag_checkbutton);
	ctk_toggle_button_set_active (GTK_TOGGLE_BUTTON (tag_checkbutton), FALSE);
	...
	ctk_container_add (GTK_CONTAINER (panel_grid), implicit_trailing_newline_checkbutton);
	...
	ctk_container_add (GTK_CONTAINER (scrolled_window), GTK_WIDGET (view));
	ctk_container_add (GTK_CONTAINER (hgrid), scrolled_window);

	ctk_container_add (GTK_CONTAINER (window), hgrid);
